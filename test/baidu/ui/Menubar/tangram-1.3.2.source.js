...
baidu.dom.getPosition = function (element) {
    ...
    if(element.getBoundingClientRect){ // IE and Gecko 1.9+
        ...
        htmlBorderLeftW = parseInt(getStyle(htmlDom, 'borderLeftWidth'));
        htmlBorderTopW = parseInt(getStyle(htmlDom, 'borderTopWidth'));
 ...
 * @param {Object} options optional     其他可选项.
 * @config {String} charset             script的字符集
 * @config {String} queryField         callback请求的url query field，默认为callback
 */
...
(function(){
    ...
    baidu.dom.drag = function(element, options) {
        ...
        //console.log("start captureEvents for id:" + target.id);
        // 设置鼠标粘滞
    ...
    function stop() {
        ...
        //console.log("drag releaseCapture");
        // 解除鼠标粘滞
        ...
        baidu.event.un(document, "mouseup", stop);
 ...
 * @param {boolean} optional 是否递归包装（如果数组里面一项仍然是数组，递归），可选
 *
...
(function(){
    var isArray = baidu.lang.isArray,
    ...
    proto['each'] = function(iterator){
        ...
        baidu.object.each(this._dom, function(node){
            iterator.call(this, (new Element(node)));
        });
    };

    //包装dom包下的大部分方法，除了drag,ready,create,ddManager
    baidu.each(("addClass draggable empty hide show insertAfter insertBefore insertHTML removeClass " + 
              ...
              "last next prev contains getAttr getPosition getStyle g q query hasClass intersect " + 
              "remove droppable removeStyle getText hasAttr").split(' '),
              function(fn){
                  proto[fn] = proto[fn.replace(/^get[A-Z]/g, stripGet)] = wrapFn(baidu.dom[fn]);
              });

    //包装event中的on 和 un
    baidu.each(("on un").split(' '), function(fn){
        proto[fn] = wrapFn(baidu.event[fn]);
    });
    //包装event的快捷方式
    ...
    baidu.e = baidu.element = function(node){
        return new Element(baidu._g(node));
    };
     ...
     * @return {function};
     */
    function wrapFn(func){
        return function(){
            var ret = baidu.fn.methodize(baidu.fn.multize(func), "_dom").apply(this, arguments);
...
/**
 * 使元素可以容纳被拖拽的DOM元素
 * @param   {HTMLElement|ID}    element 容器元素
 * @param   {JSON}              options 拖曳配置项
 *          {
 *          	ondrop,
 *           	ondropover,
...
baidu.dom.removeStyle = function (){
    ...
    if (ele.style.removeProperty) {// W3C, (gecko, opera, webkit)
        fn = function (el, st){
            return _g(el).style.removeProperty(st);
        };
    } else if (ele.style.removeAttribute) { // IE
        fn = function (el, st){
            return _g(el).style.removeAttribute(baidu.string.toCamelCase(st));
        };
    }
    ele = null;
    return fn;
...
/**
 * 接受两个数组参数，返回一个对象
 * 这个对象使用第一个数组做为key，使用第二个数组做为值
 * 如果第二个参数未指定，则把对象的所有值置为true
 *
 * @method hash
 * @static
 * @param keys {Array} key数组
 * @param values {Array} optional 值数组
 * @return {object} hash数组
 */
...
/** 
 * 返回一个封装后的函数，这个函数会在指定的作用域执行原函数
 *
 * @method baidu.fn.bind
 * @param f {Function|String} 要绑定的函数，或者一个在作用域下可用的函数名
 * @param c {object} optional 执行作用域，如果不传此参数
 * 则将这个函数的作用域从书写上下文中独立出来
 * @param args* 0..n 函数执行时附加到执行时函数前面的参数
 * @return {function} 封装后的函数
 */
baidu.fn.bind = function(f, c) {
    var xargs = arguments.length > 2 ? [].slice.call(arguments, 2) : null;
    return function () {
        var fn = baidu.lang.isString(f) ? c[f] : f,  
...
baidu.event._eventFilter.mouseenter = window.attachEvent ? null : function(element,type, listener){
	//仅在鼠标初次进入元素区域时触发,
	//需要过滤掉子元素冒泡上来的mouseover.
	function mouseEnter(e) {
		var parent = e.relatedTarget,
			current = e.currentTarget;
		// Firefox sometimes assigns relatedTarget a XUL element
		// which we cannot access the parentNode property of - from jQuery and Mooltools
		if(parent.prefix == 'xul'){return;}
 ...
 * @param {HTMLElement|string|window} element  目标元素或目标元素id
 * @param {string}                    type     事件类型
 * @param {Object}                    options  触发参数
 * @return {HTMLElement} 目标元素
 * 
 * 触发参数详细说明(W3C)
 * options = {
 * 	bubbles : true|false 是否冒泡
 * 	cancelable : true|false 是否可以阻止事件的默认操作
 * 	view : window|null 指定 Event 的 AbstractView
 * 	detail : 1|number 指定 Event 的鼠标单击量
 * 	screenX : number 指定 Event 的屏幕 x 坐标
 * 	screenY : number 指定 Event 的屏幕 y 坐标
 * 	clientX : number 指定 Event 的客户端 x 坐标
 * 	clientY : number 指定 Event 的客户端 y 坐标
 *  ctrlKey : true|false指定是否在 Event 期间按下 ctrl 键
 * 	altKey : true|false指定是否在 Event 期间按下 alt 键
 * 	shiftKey : true|false指定是否在 Event 期间按下 shift 键
 * 	metaKey : true|false指定是否在 Event 期间按下 meta 键
 * 	button : number 指定 Event 的鼠标按键
 * 	keyCode : number
 * 	charCode : number
 * 	relatedTarget : html-element 指定 Event 的相关 EventTarget
 * }
 */
(function(){
	var keys = {
	...
	baidu.event.fire = function(element, type, options){
		...
		}else if(mouses.hasOwnProperty(type)){
			evnt = mouseEvents(type, options);
		}else if(htmls.hasOwnProperty(type)){
