...
test('default properties', function() {
	expect(3);
	var m = new baidu.ui.modal.Modal();
	m.render();
...
test('窗口resize测试（缩小）', function() {
	...
	ua.frameExt(function(w, f) {
		...
		var me = this;
		var m = new w.baidu.ui.modal.Modal();
		var flag = false;
...
test('部分遮罩（在iframe中）', function() {
	...
	ua.frameExt(function(w, f) {
		...
		w.$(tt).css('width', 200).css('height', 200).css('background-color',
				'red');
		var m1 = new w.baidu.ui.modal.Modal({
			container : tt
		});
		m1.render();
...
test('部分遮罩（在windows中）', function() {
	...
	$(tt).css('width', 200).css('height', 200).css('background-color',
			'red');
	var m1 = new baidu.ui.modal.Modal({
		container : tt
	});
	m1.render();
	m1.show();// 标准模式下，此处计算位置会出错 PUBLICGE-221
...
test('hide flash', function() {
	...
	var check = function(){
		...
		ok(Math.abs(m.getMain().offsetWidth - baidu.page.getViewWidth()) < 5, 'check width after shown');
    ...
    ua.importsrc('baidu.swf.create', 
			check ,'baidu.swf.create', 'baidu.ui.modal.Modal');
});
