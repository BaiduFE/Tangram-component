...
test('mousedown prev next',function(){
	...
	ua.loadcss(upath+'ScrollBar/style.css',function(){
		...
		ua.mouseup(scrollbar._prev.getBody());
		...
		ua.mouseup(scrollbar._slider.getThumb());
...
test('flushUI',function(){
	...
	ua.loadcss(upath+'ScrollBar/style.css',function(){
	    ...
	    equal(baidu.dom.getStyle(thumb,'top'),Math.round(sliderheight*0.5-thumbheight/2)+'px','flushUI thumb top');
	    te.obj.push(scrollbar);
		start();
   });
});
