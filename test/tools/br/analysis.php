...
class Analysis{
	...
	public function Analysis($proj='Tangram-base'){
		...
		if(sizeof(self::$projpath) == 0){
			...
			array_push(self::$projpath, "../../../../Tangram-component/src");
...
//		new Analysis();
		if(!array_key_exists($domain, self::$_cache)){
			...
			foreach(self::$projpath as $i=>$p){
				$pp = $p.'/'.$path;
				//				print $pp;
				if(file_exists($pp)){
					$cnt = file_get_contents($pp);
					break;
				}else if(self::$DEBUG)
				echo 'file not exists : '.$pp;
