 ...
 * @param {String}              [config.items.type="toolbar-button"]ui控件类型
...
baidu.ui.toolbar.Toolbar = baidu.ui.createUI(function (options){
     ...
     * @property
     * */
    tplMain:'<div id="#{mainId}" class="#{mainClass}">' +
            '#{title}'+
    ...
    add:function(options,container){
        ...
        tmpNS = options.type.split("-");
        if(tmpNS.length == 1){
            tmpNS = tmpNS.shift();
            uiNS = baidu.ui[tmpNS.charAt(0).toUpperCase() + tmpNS.slice(1)];
        }else{
            uiNS = baidu.ui[tmpNS.shift()];
            baidu.each(tmpNS,function(item){
                tmpNS.length == 1 && (item = item.charAt(0).toUpperCase() + item.slice(1));
                uiNS = uiNS[item];
            });
        }
        ...
        //若uiInstance成功创建
        if(uiInstance){
            container = baidu.g(container);
            if(!container){
                container = me._createCell(1,"html")[0];
            }

            uiInstance.render(container);
            me._itemObject[uiInstance.name] = uiInstance;
        }
        return uiInstance;
    },
     ...
     * @returns {String|HTMLElement} cell
     * */
