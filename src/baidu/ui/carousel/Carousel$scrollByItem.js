...
/**
 * item和page插件的实现初忠是移动和焦点分离，用户可只实现翻页而不定位焦点，
 * 当fx引入时，在内核中占用了fx的onbeforescroll事件，则当连续点击翻页（第一次翻还同结束时）没有办法得到当次
 * 滚动是否生效，则这里只能通过定义一个onbeforestartscroll来处理滚动生效之前的动作，同时在内核中可以实现滚动
 * 时对第二参数的设置如果这里使用addEventListener进行注册，则如果同时导入item和page或是重复导文件会造成多次
 * 注册，同时执行时造成多次执行的错误
 * 
 * XXX onbeforestartscroll不能开放给用户使用,不能使用addEventListener注册
 */
baidu.ui.carousel.Carousel.prototype.onbeforestartscroll = function(evt) {
    this.focus(evt.index);
};
 ...
 * @return void.
 */
baidu.ui.carousel.Carousel.prototype.prev = function() {
    this._gotoItem('prev')
};
 ...
 * @return void.
 */
baidu.ui.carousel.Carousel.prototype.next = function() {
    this._gotoItem('next');
};
 ...
 */
baidu.ui.carousel.Carousel.prototype._gotoItem = function(pos) {
    var me = this, 
        index, 
        item;
