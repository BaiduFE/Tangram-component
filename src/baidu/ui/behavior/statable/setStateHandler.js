 ...
 * @param {DOMEvent} evnt 事件触发时的Event对象.
 */
baidu.ui.behavior.statable._statableMouseHandler = function(eventType, group, key, evnt) {
    this._fireEvent(eventType, group, key, evnt);
},

/**
 * 使用dom的形式为该节点增加事件
 * @param {HTMLElement} element 事件源.
 * @param {Object} group 状态类型，同一类型的相同状态会被加上相同的css.
 * @param {Object} key 索引，在同一类中的索引.
 * @memberOf {TypeName}
 * @return {Object} 格式：{evntName0 : handler0, evntName1 : handler1}.
 */
baidu.ui.behavior.statable.setStateHandler = function(element, group, key) {
    var me = this, 
        handler = {};
